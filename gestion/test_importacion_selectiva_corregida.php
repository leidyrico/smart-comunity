...
echo "=== PRUEBA DE IMPORTACIÓN SELECTIVA DE RECIBOS VENCIDOS (CORREGIDA) ===\n\n";
...
try {
    ...
    // Crear archivo Excel de prueba con estructura completa
    echo "Creando archivo Excel de prueba...\n";
    $excelData = [
        ['numero_recibo', 'periodo', 'fecha_emision', 'fecha_vencimiento', 'valor_administracion', 'valor_aseo', 'valor_vigilancia', 'valor_mantenimiento', 'otros_conceptos', 'total_recibo', 'estado', 'observaciones'],
        ['REC-SEL-001', '2024-01', '01/01/2024', '31/01/2024', 40000, 5000, 3000, 2000, 0, 50000, 'vencido', 'Recibo selectivo 1'],
        ['REC-SEL-002', '2024-02', '01/02/2024', '28/02/2024', 45000, 5000, 3000, 2000, 0, 55000, 'vencido', 'Recibo selectivo 2']
    ];
    ...
    $testFile = __DIR__ . '/test_recibos_selectivos_corregido.xlsx';
    ...
    for ($i = 1; $i < count($data); $i++) {
        ...
        // Crear el recibo con todos los campos obligatorios
        $recibo = ReciboGastoComun::create([
            ...
            'valor_administracion' => (float)$rowData['valor_administracion'],
            'valor_aseo' => (float)$rowData['valor_aseo'],
            'valor_vigilancia' => (float)$rowData['valor_vigilancia'],
            'valor_mantenimiento' => (float)$rowData['valor_mantenimiento'],
            'otros_conceptos' => (float)$rowData['otros_conceptos'],
        ...
        echo "  Desglose: Adm=\${$recibo->valor_administracion}, Aseo=\${$recibo->valor_aseo}, Vig=\${$recibo->valor_vigilancia}, Mant=\${$recibo->valor_mantenimiento}, Otros=\${$recibo->otros_conceptos}\n";
    ...
    // Verificar que solo los apartamentos seleccionados tienen asignaciones
    $apartamentosConAsignaciones = Pago::where('observaciones', 'LIKE', '%Asignación selectiva%')
        ->distinct('apartamento_id')
        ->count();
    
    echo "\n=== VALIDACIÓN DE SELECTIVIDAD ===\n";
    echo "Apartamentos seleccionados: " . count($apartamentosSeleccionados) . "\n";
    echo "Apartamentos con asignaciones: {$apartamentosConAsignaciones}\n";
    
    if ($apartamentosConAsignaciones == count($apartamentosSeleccionados)) {
        echo "✓ CORRECTO: Solo los apartamentos seleccionados recibieron asignaciones\n";
    } else {
        echo "✗ ERROR: Hay discrepancia en las asignaciones\n";
    }
    
    echo "\n=== PRUEBA COMPLETADA EXITOSAMENTE ===\n";
    echo "✓ Los recibos se importaron correctamente con todos los campos\n";
    echo "✓ Solo se asignaron a los apartamentos seleccionados\n";
    echo "✓ Los estatus financieros se actualizaron apropiadamente\n";
    echo "✓ La funcionalidad de importación selectiva está funcionando\n";
